...
/*j'ai rajouter quelque ligne qui permettent de passer a la suite du programme au bout de 60 ms, 
 *ce qui permet de faire fonctionner mon chronometre en même temps que mon wait clic !*/
POINT wait_clic_chro()
	{
	int encore = 1;
	POINT P;
	SDL_Event event;
...
float ch=chrono_val(),ch2=0; int i=1;

	while ((SDL_WaitEvent(&event) && encore) || (i==1) )
		{
...
SDL_SetColorKey(image, SDL_SRCCOLORKEY, SDL_MapRGB(image->format, 242, 67, 133)); 
/* J'ai rajouté une ligne qui rend transparente la couleur 
 * de fond choisie (ici, le rose des images).
 * */
